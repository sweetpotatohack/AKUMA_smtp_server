...
Основные записи:
A     mail.$DOMAIN                    IN A     $SERVER_IP
MX    $DOMAIN                        IN MX    10 mail.$DOMAIN

SPF запись:
TXT   $DOMAIN                        IN TXT   "v=spf1 mx ~all"

DMARC запись:
TXT   _dmarc.$DOMAIN                 IN TXT   "v=DMARC1; p=none; rua=mailto:dmarc@$DOMAIN"

DKIM запись:
$DKIM_RECORD

Дополнительные записи (опционально):
CNAME webmail.$DOMAIN                IN CNAME mail.$DOMAIN
CNAME smtp.$DOMAIN                   IN CNAME mail.$DOMAIN
CNAME imap.$DOMAIN                   IN CNAME mail.$DOMAIN
CNAME pop3.$DOMAIN                   IN CNAME mail.$DOMAIN
